...
def plot_asimov_heatmap(df, folder, title=f"Asimov Significance (Z>2.0)"):
    ...
    y = df['m_LSP'].values
    z = df['Z'].values  # Asimov significance Z of each signal point

    x_min, x_max = x.min(), x.max()
    y_min, y_max = y.min(), y.max()

    xi = np.arange(x_min, x_max + 1, 10)
    yi = np.arange(y_min, y_max + 1, 10)
    xi, yi = np.meshgrid(xi, yi)

    # Interpolate the significance value
    zi = griddata((x, y), z, (xi, yi), method='cubic', fill_value=0)

    binary_zi = (zi > 2.0).astype(int)
    area = np.sum(binary_zi)
    total_pixels = binary_zi.size
    auep_score = area / total_pixels

    zi_smooth = gaussian_filter(zi, sigma=2)

    levels = [0, 0.5, 1, 2, 5, 10, 20, 30, 50, 70, 100, 200, 400]
    visible_levels = [0, 1, 5, 20, 50, 100, 400]

    cmap = plt.get_cmap('viridis')
    norm = colors.BoundaryNorm(levels, ncolors=cmap.N, clip=True)

    zi_plot = np.clip(zi_smooth, 0, max(levels))

    _, ax = plt.subplots(figsize=(10, 8))

    cf = ax.contourf(xi, yi, zi_plot, levels=levels, cmap=cmap, norm=norm)
    cs = ax.contour(xi, yi, zi_plot, levels=levels, colors='black', linewidths=0.5, alpha=1)
    ax.clabel(cs, levels=levels, inline=True, fontsize=10, fmt='%g', colors='black')

    cbar = plt.colorbar(cf, ax=ax, ticks=visible_levels)
    cbar.ax.set_yticklabels([str(l) for l in visible_levels])
    cbar.set_label('Asimov Significance $Z$', rotation=90, labelpad=20)

    ax.contour(xi, yi, zi_smooth, levels=[2.0], colors='red', linewidths=2.5, linestyles='--')

    text_str = f"AUEP: {auep_score:.3f}"
    ...
    plt.tight_layout()
    plt.savefig(f"{folder}/asimov_heatmap_{sig_type}.png")
    plt.close()
