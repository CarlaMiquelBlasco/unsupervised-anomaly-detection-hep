...
def plot_latent_distribution(z, out_path=None):
    ...
    plt.title("Latent distribution z ~ N(0,1)?")
    plt.xlabel("z")
    plt.grid(True, ls="--", alpha=0.5)
...
def plot_logdet_distribution_bkg_vs_sig(logdet_bkg, logdet_sig, out_path=None):
    plt.figure(figsize=(7,5))
    sns.histplot(logdet_bkg, bins=60, kde=True, color="steelblue", label="Background", alpha=0.5)
    sns.histplot(logdet_sig, bins=60, kde=True, color="orange", label="Signal", alpha=0.5)
    plt.xlabel("log det(J)")
    plt.ylabel("Count")
    plt.title("Jacobian log-det — Background vs Signal")
    plt.grid(True)
    plt.legend()
    if out_path:
        plt.savefig(out_path, dpi=200)
...
def plot_logdet_distribution_bkg_by_class_vs_sig(
    logdet_bkg, class_bkg, logdet_sig, out_path=None
):
    """
    Plot the distribution of log|det J|:
    - Background split by physics Class
    - Signal overlaid in black
    ...
    logdet_bkg = np.asarray(logdet_bkg)
    class_bkg = np.asarray(class_bkg)
    logdet_sig = np.asarray(logdet_sig)

    # Replace integer labels with physics names
    class_names = np.array([class_map.get(c, f"Class {c}") for c in class_bkg])

    # Unique classes in background
    ...
    # Plot background class-by-class
    ...
    plt.xlabel("log det(J)")
    plt.ylabel("Count")
    plt.title("Jacobian log-det — Background by Physics Class vs Signal")
    plt.grid(True, ls="--", alpha=0.5)
    plt.legend()
    plt.tight_layout()
...
# --------------------------------------------------------------
# 7. FEATURE-WISE -logp(x) HEATMAP (NF-specific)
# --------------------------------------------------------------

def plot_feature_contributions(df, score_col, out_path=None):
    """
    Correlates each input variable with the NF score (-logp).
    Useful for interpretability.
    """

    corr = df.corr()[score_col].sort_values()

    plt.figure(figsize=(8,10))
    sns.barplot(x=corr.values, y=corr.index)
    plt.xlabel("Correlation with -log p(x)")
    plt.title("Feature contributions to NF anomaly score")
