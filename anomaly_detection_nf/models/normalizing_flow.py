...
def build_flow(input_dim, config):
    """
    ...
    for i in range(num_layers):
        ...
        if flow_type == "affine_coupling":
            transform = transforms.AffineCouplingTransform(
                mask=torch.arange(0, input_dim) % 2,  # half frozen, half transformed
        ...
        # Neural Spline Flow Coupling
            ...
            transform = transforms.PiecewiseRationalQuadraticCouplingTransform( # funtion from nflows library
                mask=torch.arange(0, input_dim) % 2,  #  half the features are frozen, and the other half are transformed
                transform_net_create_fn=lambda in_features, out_features: ResidualNet( # neural network that predicts spline parameters (MLP)
                    in_features=in_features, # frozen part of the data (input_dim/2)
        ...
        transforms_list.append(transforms.RandomPermutation(features=input_dim)) # we permute the order of features randomly to not froze/transformalways the same features
